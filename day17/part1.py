        ...
        for y in range(len(initial_plane)):
            for x in range(len(initial_plane[0])):
                if initial_plane[y][x] == "#":
                    ...
                    y_grid = cycles + y
                    x_grid = cycles + x
                    cube = self.grid[z_grid][y_grid][x_grid]
                    cube.active = True
            ...
            for y_offset in range(-1, 2):
                for x_offset in range(-1, 2):
                    try:
                        ...
                        if (x_neigh < 0) or (y_neigh < 0) or (z_neigh < 0):
                            raise IndexError
                        ...
                        neigh_cube = self.grid[z_neigh][y_neigh][x_neigh]
        ...
        for z in range(len(self.grid)):
            for y in range(len(self.grid[0])):
                for x in range(len(self.grid[0][0])):
                    cube = new_grid[z][y][x]
                    active_neighs = self.get_active_neighbours(x, y, z)
