...
class Tiles:
    ...
    def run_day_v2(self):
        # Key is coordinate, the value is the number of black tiles surround it
        black_neighbours = {}

        new_black_tiles = set()

        # Loop through all black neighbours and calculate the number of
        # black tiles surrounding each neighbour
        #
        # Note, all the black tiles that will be alive next round have
        # to have at least 1 black neighbours, so "black tiles alive next day"
        # will be a subset of "the neighbours of all the black tiles"
        for black_tile in self.black_tiles:
            neighbours = self.get_surr_tiles(black_tile)
        ...
        for neighbour in black_neighbours:
            if black_neighbours[neighbour] == 2:
                new_black_tiles.add(neighbour)
            elif (black_neighbours[neighbour] == 1) and (neighbour in self.black_tiles):
                new_black_tiles.add(neighbour)

        self.black_tiles = new_black_tiles

    def run_days_v2(self, days):
        for _ in range(days):
            self.run_day_v2()
            # print(_)
